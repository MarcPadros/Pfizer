...
CreateConceptDatasets <- function(codesheet,file, c.voc, c.concept, c.codes, c.startwith = NULL, f.code, f.voc, path, method = "SQL", group = T, f.name = NULL ){
  ...
  if(method == "SQL"){
    ...
    }else{saveRDS(object = TEMP1, file = paste0(path,"/",f.name,".rds"))}
