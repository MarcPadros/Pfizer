...
MATCHED <- do.call(rbind, MATCHED_TEMP)[,.(Exposed,Control,T0)]
MATCHED <- MATCHED[, id := row.names(MATCHED)]
MATCHED <- MATCHED[, T0 := as.Date(T0, origin="1970-01-01")]

saveRDS(MATCHED,paste0(populations_dir,"MATCH_PAIRS_SQL.rds"))
